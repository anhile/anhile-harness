#!/usr/bin/env bash
#
# The verification gate. Runs nine steps in order:
#
#   1. eslint                 (layer boundaries + code rules)
#   2. tsc -b                 (all packages, via project references)
#   3. jest unit              (contracts, api, web)
#   4. jest API e2e           (supertest against a booted Nest app + test DB)
...
#
# Mutation testing was step 10. It found two unpinned bounds on its first run
# and nothing across the fifty-six runs after, so it is `pnpm mutation` now —
# run it when the tests change, and weekly in CI alongside the witness job.
# A gate that has never once said no is documentation with a stopwatch on it.
#
...
write_receipt() {
  node scripts/verify-receipt.mjs write \
    ...
    --failed "$2" >/dev/null
  # The durable half. .generated/runs/ is machine-local and 64 MB deep; this one line
  # per run is what survives in git. Failing runs are appended too -- a record
  # that keeps only the green runs is not a record.
...
# documented numbers, so nothing anybody has memorised moves.
#
...
if [[ "$(git rev-parse --git-dir 2>/dev/null)" != "$(git rev-parse --git-common-dir 2>/dev/null)" ]]; then
  ...
  # right and does nothing: `.env` is copied from `.env.example`, which always
  # sets TEST_DATABASE_URL, so the variable was never empty and the derivation
  # never fired. The gate then went green against the shared database while
  # appearing to prove isolation — twice, on 2026-09-11, the second time in
  # the fix for the first.
  VERIFY_API_PORT="${_caller_api_port:-$(( 3100 + _slot ))}"
  VERIFY_WEB_PORT="${_caller_web_port:-$(( 5273 + _slot ))}"
  # The suffix goes in the middle, not at the end: migrate.mjs honours --yes
  # only for a name ending in `_test` (I8), so `shortener_test_w7` would lose
  # the protection the whole scheme depends on.
  TEST_DATABASE_URL="${_caller_test_db:-postgres://shortener:shortener@localhost:5433/shortener_w${_slot}_test}"
  unset _slot
fi
...
export COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-link-shortener}"

export API_PORT="${API_PORT:-3000}"
export WEB_PORT="${WEB_PORT:-5173}"
export DATABASE_URL="${DATABASE_URL:-postgres://shortener:shortener@localhost:5433/shortener}"
export TEST_DATABASE_URL="${TEST_DATABASE_URL:-postgres://shortener:shortener@localhost:5433/shortener_test}"
export VERIFY_API_PORT="${VERIFY_API_PORT:-3100}"
export VERIFY_WEB_PORT="${VERIFY_WEB_PORT:-5273}"
...
ensure_database() {
  ...
  for _ in $(seq 1 60); do
    ...
    # is the only one that binds TCP. CI hit this; see PROGRESS.md 2026-08-30.
    docker compose exec -T postgres pg_isready -h 127.0.0.1 -U shortener -d shortener >/dev/null 2>&1 && return 0
    sleep 1
  done
  return 1
}
...
prepare_test_database() {
  ...
  # literal. Those two disagreed: the literal created `shortener_test` while
  # the migration ran against whatever the URL said, so a worktree pointed at
  # its own database got a migration against one that had never been created.
  local db="${TEST_DATABASE_URL##*/}"; db="${db%%\?*}"
  case "$db" in
    *_test) ;;
    *) echo "refusing: TEST_DATABASE_URL names '$db', which does not end in _test"; return 1 ;;
  esac
  docker compose exec -T postgres psql -U shortener -d shortener -tc \
    "select 1 from pg_database where datname = '$db'" | grep -q 1 \
    || docker compose exec -T postgres createdb -U shortener "$db"
  # --yes is safe here and only here: a *_test database is disposable, and the
...
browser_e2e() {
  # Against the disposable test database, not the dev one. verify.sh used to
  # lean on `shortener` being migrated, which is init.sh's doing -- so the gate
  # passed on a machine that had run init.sh and failed on a clean checkout.
  # CI found that on its first run. Reusing shortener_test keeps verify.sh
  # self-contained and keeps it out of the developer's data entirely; it is
  # still the only database --yes is ever passed for, so I8 is unchanged.
  prepare_test_database || return 1

  # Dedicated ports, and this is not cosmetic. Playwright reuses a server that
  # is already listening (`reuseExistingServer`, so ./init.sh and ./verify.sh do
  # not fight over 3000). On the default ports that means a dev API pointed at
  # the developer's own database gets handed the tests -- while the expiry
  # helper writes to shortener_test through DATABASE_URL. Two databases, and
  # only the tests that touch one directly notice: five of them.
