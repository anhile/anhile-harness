...
- The commit gate asks a spike for no receipt: green, red or absent, the
  commit goes through, and stderr says the branch is a spike. The
  protected-file checks (I11) run before that decision and are unchanged.
...
- **AC1** (ubiquitous) — On a `spike/*` branch the commit gate shall let a
  commit through with no receipt, or a red one, and shall still refuse an
  edit to a protected file.
...
|---|---|---|---|
| AC1 | Jest — `commit-gate.spec.ts` "a spike branch is asked for no receipt (I16)": no receipt, red receipt, protected edit | the cases pass | `.generated/runs/<ts>/03-unit.log` |
| AC2 | Jest — `session-hooks.spec.ts` "session-stop lets a spike stop with commits and no journal entry, and no push reminder" | the case passes | same |
| AC3 | Jest — `session-hooks.spec.ts` "session-start says what a spike is, once, under the branch line"; `spike.spec.ts` pins the note's sentences | the cases pass | same |
...
| AC5 | Jest — `commit-gate.spec.ts` "asks the receipt again the moment the branch is not a spike"; `session-hooks.spec.ts` "asks again on a branch that is not under spike/"; `spike.spec.ts` "is any branch under spike/, and nothing else" | the cases pass | same |
| AC6 | Jest — `harness-init.spec.ts` "copies the scripts the gate calls…" asserts `scripts/spike.mjs`; the seed `AGENTS.md` text is in `harness-init.mjs` beside the table the existing seed case reads | the cases pass | same |
...
- tests: `commit-gate.spec.ts`, `session-hooks.spec.ts`, `pr-ready.spec.ts`, `ci-workflow.spec.ts`, `harness-init.spec.ts` — fixtures gain `spike.mjs` beside the scripts that import it; no case weakened
