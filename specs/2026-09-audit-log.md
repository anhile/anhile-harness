...
- `audit-log.mjs check`: no file edited, removed or misnamed, every file
  ...
  gate before every commit, as verify-log's guard is, and by step 07, whose
  `verify-log.mjs check` asks the same of both records.
...
- **AC1** (event-driven) — When `audit-receipt.mjs write` records a verdict,
  ...
  receipt's fields, for every verdict and not only READY; and if the append
  is refused — a name already taken, a directory that cannot be written —
  then no receipt shall be left for the gate to read.
...
- **AC3** (unwanted) — If a commit flips an entry's `passes` to true and its
  ...
  audits it did find said. An entry that records no contract (`spec: null`,
  the shape a list from before contracts has) is closed by a READY audit of
  the commit's tree under any contract, as the gate's receipt rule already
  reads it; no commit is exempt.
...
|---|---|---|---|
| AC1 | Jest — `audit-receipt.spec.ts` "write › appends the verdict to audit-log/, whatever it is" (READY and NOT_READY, the name from `at`, the fields) and "write › leaves no receipt when the log refuses the append"; `audit-log.spec.ts` "append: what the log keeps", including "never a second file under one name" | the cases pass | `.generated/runs/<ts>/03-unit.log` |
| AC2 | Jest — `audit-log.spec.ts` "what the log refuses" (rewritten, removed, stray, not JSON, missing fields, dated before its name); `commit-gate.spec.ts` "refuses a commit when a recorded audit has been rewritten", "…removed", each naming the file; `verify-log.spec.ts` "step 07 asks the same of the audits" | the cases pass | `.generated/runs/<ts>/03-unit.log` |
| AC3 | Jest — `audit-log.spec.ts` "a committed closure carries its audit": no audit, a READY audit of this tree, NOT_READY, another tree, another contract, an entry with no contract, each through `check-feature-list.mjs --at --base` on a real commit; `ci-workflow.spec.ts` still walks a two-commit sequence | the cases pass | `.generated/runs/<ts>/03-unit.log` |
| AC4 | Jest — `audit-log.spec.ts` "the tree of a commit hashes as the receipt hashed it", with an executable file and a symlink in the commit; "leaves out the two records and nothing else", pinned on `tree` itself rather than by comparison with a hasher that shares its skip list; "this repository's own HEAD is a tree some recorded run covers", the same arithmetic against the real record | the cases pass | `.generated/runs/<ts>/03-unit.log` |
...
- `scripts/check-feature-list.mjs`, `scripts/verify-log.mjs` (step 07 asks the audits' guard too), `scripts/harness-init.mjs` (one line of the AGENTS seed)
- `verify-log/` — the run records the gate writes on the way, a by-product
...
- tests: `scripts/__tests__/audit-log.spec.ts`, added; `audit-receipt.spec.ts`, `commit-gate.spec.ts`, `attestation.spec.ts`, `ci-workflow.spec.ts`, `feature-list.spec.ts`, `verify-log.spec.ts`, and every fixture that copies `verify-log.mjs` — fixtures gain the module the patched scripts import, and the cases named above. No case weakened.
